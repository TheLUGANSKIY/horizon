...
// that submits directly to the TheLUGANSKIY-core at `url` using the http client
// `h`.
...
// coreSubmissionResponse is the json response from TheLUGANSKIY-core's tx endpoint
...
// submits directly to the configured TheLUGANSKIY-core instance using the
...
// Submit sends the provided envelope to TheLUGANSKIY-core and parses the response into
// a SubmissionResult
func (sub *submitter) Submit(ctx context.Context, env string) (result SubmissionResult) {
	...
	// interpet response
	if cresp.Exception != "" {
		result.Err = errors.Errorf("TheLUGANSKIY-core exception: %s", cresp.Exception)
		return
	}
	...
	default:
		result.Err = errors.Errorf("Unrecognized TheLUGANSKIY-core status response: %s", cresp.Status)
	}
