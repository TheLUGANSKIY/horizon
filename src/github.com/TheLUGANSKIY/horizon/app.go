...
import (
	...
	"github.com/TheLUGANSKIY/go/build"
	...
	"github.com/TheLUGANSKIY/horizon/paths"
	"github.com/TheLUGANSKIY/horizon/reap"
...
// CoreSession returns a new session that loads data from the TheLUGANSKIY core
// database. The returned session is bound to `ctx`.
...
func (a *App) UpdateStellarCoreInfo() {
	...
	fail := func(err error) {
		log.Warnf("could not load TheLUGANSKIY-core info: %s", err)
	}
	...
	// TODO: make resilient to changes in TheLUGANSKIY-core's info output
...
func (a *App) Tick() {
	...
	// update ledger state and TheLUGANSKIY-core info in parallel
	wg.Add(2)
