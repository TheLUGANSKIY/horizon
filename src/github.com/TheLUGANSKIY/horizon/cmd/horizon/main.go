...
import (
	...
	"github.com/TheLUGANSKIY/horizon"
	hlog "github.com/TheLUGANSKIY/horizon/log"
)
...
func init() {
	...
	viper.BindEnv("TheLUGANSKIY-core-db-url", "STELLAR_CORE_DATABASE_URL")
	viper.BindEnv("TheLUGANSKIY-core-url", "STELLAR_CORE_URL")
	...
	rootCmd = &cobra.Command{
		Use:   "horizon",
		Short: "client-facing api server for the TheLUGANSKIY network",
		Long:  "client-facing api server for the TheLUGANSKIY network",
	...
	rootCmd.Flags().String(
		"TheLUGANSKIY-core-db-url",
		"",
		"TheLUGANSKIY-core postgres database to connect with",
	)

	rootCmd.Flags().String(
		"TheLUGANSKIY-core-url",
		"",
		"TheLUGANSKIY-core to connect with (for http commands)",
	)
	...
	rootCmd.Flags().Bool(
		"ingest",
		false,
		"causes this horizon process to ingest data from TheLUGANSKIY-core into horizon's db",
	)
	...
	rootCmd.Flags().Uint(
		...
		0,
		"the maximum number of ledgers the history db is allowed to be out of date from the connected TheLUGANSKIY-core db before horizon considers history stale",
	)
...
func initConfig() {
	...
	if viper.GetString("TheLUGANSKIY-core-db-url") == "" {
		log.Fatal("Invalid config: TheLUGANSKIY-core-db-url is blank.  Please specify --TheLUGANSKIY-core-db-url on the command line or set the STELLAR_CORE_DATABASE_URL environment variable.")
	}

	if viper.GetString("TheLUGANSKIY-core-url") == "" {
		log.Fatal("Invalid config: TheLUGANSKIY-core-url is blank.  Please specify --TheLUGANSKIY-core-url on the command line or set the STELLAR_CORE_URL environment variable.")
	}
	...
	config = horizon.Config{
		...
		StellarCoreDatabaseURL: viper.GetString("TheLUGANSKIY-core-db-url"),
		StellarCoreURL:         viper.GetString("TheLUGANSKIY-core-url"),
