...
import (
	...
	"github.com/TheLUGANSKIY/go/support/db"
...
// Offer is row of data from the `offers` table from TheLUGANSKIY-core
type Offer struct {
...
// Q is a helper struct on which to hang common queries against a TheLUGANSKIY
// core database.
type Q struct {
	*db.Session
}
...
// Signer is a row of data from the `signers` table from TheLUGANSKIY-core
type Signer struct {
	Accountid string
	Publickey string
	Weight    int32
...
// Transaction is row of data from the `txhistory` table from TheLUGANSKIY-core
...
// TransactionFee is row of data from the `txfeehistory` table from TheLUGANSKIY-core
...
// Trustline is a row of data from the `trustlines` table from TheLUGANSKIY-core
...
// TheLUGANSKIY-core database this ingestion system is communicating with.  Horizon,
...
// database.
//
// Due to the design of TheLUGANSKIY-core, ledger 1 will _always_ be in the database,
