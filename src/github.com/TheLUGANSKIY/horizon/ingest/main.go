...
// data produced by the connected TheLUGANSKIY-core database, transforms it and
...
// Cursor iterates through a TheLUGANSKIY core database's ledgers
type Cursor struct {
	...
	// attempt to be ingested in this session.
	LastLedger int32
	// DB is the TheLUGANSKIY-core db that data is ingested from.
	DB *db.Session
...
type System struct {
	...
	// CoreDB is the TheLUGANSKIY-core db that data is ingested from.
	CoreDB *db.Session
	...
	// StellarCoreURL is the http endpoint of the TheLUGANSKIY-core that data is being
	// ingested from.
...
// New initializes the ingester, causing it to begin polling the TheLUGANSKIY-core
